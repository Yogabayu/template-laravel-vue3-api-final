                    ...
                    <v-bottom-sheet max-width="400">
                        ...
                        <v-card>
                            ...
                            <v-card-text class="py-2 px-4">
                                Tahap ini meliputi pengumpulan dan verifikasi berkas kredit
                                nasabah. Proses ini dilaksanakan setelah Account Officer (AO)
                                melakukan survei lapangan. Dokumentasi yang terkumpul akan
                                digunakan untuk analisis SLIK (Sistem Layanan Informasi
                                Keuangan) guna menilai kelayakan kredit.
                            </v-card-text>
                        </v-card>
                    </v-bottom-sheet>
                </v-col>
        ...
        <div v-for="(attachment, index) in data" :key="index" class="pa-1">
            <v-row no-gutters align="center" :class="parseInt(attachment.isSecret) ? 'bg-grey-lighten-4' : ''">
                ...
                <v-col>
                    ...
                    <div v-if="attachment.note" class="text-caption text-grey-darken-1">
                        {{ attachment.note }}
                    </div>
                </v-col>
                ...
                <v-col cols="auto">
                    <div class="d-flex">
                        <v-tooltip location="top" text="Lihat File" v-if="canViewFileA(attachment)">
                            <template v-slot:activator="{ props }">
                                <v-btn v-bind="props" icon="bx-link-external" size="small" color="primary"
                                    variant="text" class="mr-2" :href="getFileUrlA(attachment)" target="_blank"
                        ...
                        <v-tooltip location="top" text="Edit File" v-if="
                            userAccessPhase1 &&
                            parseInt(userAccessPhase1.canUpdateData) &&
                            phase < 2
                        ">
                            <template v-slot:activator="{ props }">
                        ...
                        <v-tooltip location="top" text="Hapus File" v-if="
                            userAccessPhase1 &&
                            parseInt(userAccessPhase1.canDeleteData) &&
                            phase < 2
                        ">
                            <template v-slot:activator="{ props }">
...
export default {
    ...
    methods: {
        canViewFileA(attachment) {
            if (attachment.path == 'null' && attachment.link == null) return false;

            const isSecretFile = Boolean(parseInt(attachment.isSecret));

            if (isSecretFile) {
                // Asumsikan userAccessPhase1 adalah properti komponen yang menunjukkan akses pengguna
                return Boolean(this.userAccessPhase1 && parseInt(this.userAccessPhase1.isSecret));
            }

            return Boolean(attachment.path || attachment.link);
        },

        getFileUrlA(attachment) {
            // if (!attachment) return '';

            if (attachment.path != 'null') {
                // Asumsikan filePath dan fileId adalah properti komponen
        ...
        canViewFile(attachment) {
            return (
                ...
                (!attachment.isSecret ||
                    (this.userAccessPhase1 && parseInt(this.userAccessPhase1.isSecret)))
            );
        },
        canEditFile(attachment) {
            return (
                this.userAccessPhase1 &&
                parseInt(this.userAccessPhase1.canUpdateData) &&
                this.phase < 2
            );
        },
        canDeleteFile(attachment) {
            return (
                this.userAccessPhase1 &&
                parseInt(this.userAccessPhase1.canDeleteData) &&
                this.phase < 2
            );
        },
        viewFile(attachment) {
